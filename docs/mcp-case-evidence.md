...
|---|---|---|---|
| `mres-close-after-reservation` | `TestMCPAdmissionCloseDuringFence` | `close_during_post_fence_callback_denies_and_retains_history` | PARTIAL |
...
before final owner admission. Both cores deny the effect and retain conservative
durable state, and Rust directly checks retained request-ID history. Go does not yet
directly assert retained nonce and request-ID history for this exact schedule, so the
case remains `PARTIAL`. The second observes closure after committed admission. Its Go
mapping also requires the authenticated duplicate test; both cores retain the outcome
and prevent duplicate execution instead of claiming rollback.
...
required tests across both cores. The checker requires a complete passing 26-test runtime
report, both pinned core revisions, the preserved owner contract, the preserved
...
The report contains all 71 case IDs. One carries current `PASS` evidence, one is
`PARTIAL`, and 69 remain `NOT_RUN`. The historical catalog field remains
`{ "NOT_RUN": 71 }` so a
consumer cannot confuse source-plan status with current runtime evidence. CI creates
...
private implementations with its stated unit and bounded-runtime assertions. `PARTIAL`
records actual execution while preserving the missing direct Go history assertion. It
does not establish independent external review, normative adoption, public API
