...
사용자 지시에 따라 새 방어 검증은 시나리오 기반 단위 테스트로 진행한다.
실제 취약점 재현 실행과 시뮬레이터 제작은 현재 범위에서 제외한다.
...
| [ ] | 3 | 재시작·복구: 저장 실패·불완전 상태·재활성화·중복 처리의 기대 동작과 판정 테스트 |
| [ ] | 4 | 종료·수신 순서: 실행 순서를 고정한 상태 전이 및 판정 테스트 |
| [ ] | 5 | 프로토콜 연결: transcript·HPKE 확인·HTTP/WS 요청/응답 연결의 판정 테스트 |
| [ ] | 6 | Guard 방어: hook 누락·시간 초과·검증 실패의 실행 차단 및 오류 보고 테스트 |
| [ ] | 7 | 단위 테스트 커버리지 정리 및 실제 코어 증거와 분리한 CI·보고서 갱신 |
