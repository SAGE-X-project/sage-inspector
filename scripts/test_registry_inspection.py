...
from inspect_registry import ROOT,validate_report,validate_scenario
class RegistryInspectionTests(unittest.TestCase):
    def test_independent_audit_detects_corruption(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for f in ('registry-records.json','registry-manifest.json'): shutil.copyfile(source/f,d/f)
            shutil.copytree(source/'registry-scenarios',d/'registry-scenarios')
            def audit(): return subprocess.run(['node',str(ROOT/'scripts/check_registry_vectors.js'),str(d)],capture_output=True,timeout=15).returncode
            self.assertEqual(audit(),0)
            raw=(d/'registry-records.json').read_bytes();suite=json.loads(raw)
            for mutate in ('verdict','signature','card'):
                changed=copy.deepcopy(suite)
                if mutate=='verdict': changed['cases'][0]['expected']['verdict']='REJECT'
                if mutate=='signature': next(c for c in changed['cases'] if c['id']=='signing-1-signature-valid')['input']['signature_hex']='00'*64
                if mutate=='card': next(c for c in changed['cases'] if c['id']=='card-valid')['input']['now']=1700000300
                (d/'registry-records.json').write_text(json.dumps(changed));self.assertNotEqual(audit(),0)
            (d/'registry-records.json').write_bytes(raw)
            f=d/'registry-scenarios/registry-freshness.json';s=json.loads(f.read_text());s['steps'][1]['effects']['authorized']=16;f.write_text(json.dumps(s));self.assertNotEqual(audit(),0)
    def test_primitive_reports(self):
        raw=(ROOT/'vectors/0.10.0/registry-records.json').read_bytes()
        report=json.loads((ROOT/'docs/evidence/registry/go/registry-records.json').read_text())
    ...
    def test_scenario_reports_require_observed_effects(self):
        raw=(ROOT/'vectors/0.10.0/registry-scenarios/registry-freshness.json').read_bytes();f=json.loads(raw)
        ...
        for kind in ('missing','effects','identity','unsupported','input'):
            ...
            if kind=='effects': changed['steps'][1]['actual']['effects']={'authorized':16}
    ...
    def test_runner_error_not_rejection(self):
        with tempfile.TemporaryDirectory() as tmp:
            p=subprocess.run([sys.executable,str(ROOT/'scripts/inspect_registry.py'),'--runner',sys.executable,'--adapter',sys.executable,'--subject','test','--revision','test','--output-dir',tmp+'/out'],capture_output=True,timeout=10)
