...
func Run(args []string, stdout, stderr io.Writer, version string) int {
	...
	if *reportPath != "" {
		...
		if a == b {
			_, _ = fmt.Fprintln(stderr, "report must not overwrite suite")
			return 2
		}
		if st1, e1 := os.Stat(*suite); e1 == nil {
			if st2, e2 := os.Stat(*reportPath); e2 == nil && os.SameFile(st1, st2) {
				_, _ = fmt.Fprintln(stderr, "report aliases suite")
				return 2
			}
		}
	}
	...
	if exe, err := os.Executable(); err == nil {
		if b, err := os.ReadFile(exe); err == nil { // #nosec G304 -- Hash the running executable returned by os.Executable.
			sub.ExecutableSHA256 = conformance.Digest(b)
		}
	}
	if *adapter != "" {
		if *subject == "" || *revision == "" {
			_, _ = fmt.Fprintln(stderr, "adapter requires subject and revision")
			return 2
		}
		...
		_, _ = fmt.Fprintln(stderr, "subject/revision/adapter-arg require adapter")
		return 2
	}
	var ids []string
	if *selection != "" {
