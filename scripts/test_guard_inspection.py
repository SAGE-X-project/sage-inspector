...
from inspect_guard import ROOT,validate_report,validate_scenario
class GuardInspectionTests(unittest.TestCase):
    def test_independent_audit_detects_corruption(self):
        with tempfile.TemporaryDirectory() as tmp:
            d=Path(tmp);source=ROOT/'vectors/0.10.0'
            for f in ('guard-records.json','guard-manifest.json'): shutil.copyfile(source/f,d/f)
            shutil.copytree(source/'guard-scenarios',d/'guard-scenarios')
            def audit(): return subprocess.run(['node',str(ROOT/'scripts/check_guard_vectors.js'),str(d)],capture_output=True,timeout=15).returncode
            self.assertEqual(audit(),0)
            raw=(d/'guard-records.json').read_bytes();suite=json.loads(raw)
            for mutate in ('verdict','signature','card'):
                changed=copy.deepcopy(suite)
                if mutate=='verdict': changed['cases'][0]['expected']['verdict']='REJECT'
                if mutate=='signature': next(c for c in changed['cases'] if c['id']=='intent-signature-valid')['input']['signature_hex']='00'*64
                if mutate=='card': next(c for c in changed['cases'] if c['id']=='intent-valid')['input']['now']=1700000300
                (d/'guard-records.json').write_text(json.dumps(changed));self.assertNotEqual(audit(),0)
            (d/'guard-records.json').write_bytes(raw)
            f=d/'guard-scenarios/guard-pending-completed.json';s=json.loads(f.read_text());s['steps'][1]['effects']['dispatch']=16;f.write_text(json.dumps(s));self.assertNotEqual(audit(),0)
    def test_primitive_reports(self):
        raw=(ROOT/'vectors/0.10.0/guard-records.json').read_bytes()
        report=json.loads((ROOT/'docs/evidence/guard/go/guard-records.json').read_text())
    ...
    def test_scenario_reports_require_observed_effects(self):
        raw=(ROOT/'vectors/0.10.0/guard-scenarios/guard-pending-completed.json').read_bytes();f=json.loads(raw)
        ...
        for kind in ('missing','effects','identity','unsupported','input'):
            ...
            if kind=='effects': changed['steps'][1]['actual']['effects']={'dispatch':16}
    ...
    def test_runner_error_not_rejection(self):
        with tempfile.TemporaryDirectory() as tmp:
            p=subprocess.run([sys.executable,str(ROOT/'scripts/inspect_guard.py'),'--runner',sys.executable,'--adapter',sys.executable,'--subject','test','--revision','test','--output-dir',tmp+'/out'],capture_output=True,timeout=10)
