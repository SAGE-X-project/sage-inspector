"""Controls for the MCP intent-signature algorithm contract."""
import copy
import tempfile
...
class SignatureBoundaryTests(unittest.TestCase):
    def test_contract_and_unchecked_audit(self):
        ...
        self.assertEqual(value['boundary']['accepted'], 'ed25519')
        self.assertEqual(value['boundary']['rejected'], ['ecdsa-p256-sha256', 'secp256k1'])
    ...
    def test_contract_rejects_mutation(self):
        ...
        changes = (
            ...
            lambda item: item['boundary'].update(accepted='secp256k1'),
            lambda item: item['boundary'].update(rejected=['ecdsa-p256-sha256']),
            lambda item: item['cores']['go'].update(test='other'),
