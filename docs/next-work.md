...
1. transcript 바인딩을 지원하는 코어 API 연결 후 재실행.
2. 완성된 0.10.0 HPKE 확인 및 HTTP/WS 양방향 요청·응답 교환.
3. 실제 만료·재시작 복구 연결. 종료·수신 경합은 Go 데이터 경합 재현(FAIL), Rust 직접 병행 종료 바인딩 UNSUPPORTED로 기록했다. 코어 개선 후 재검증한다. 코어 간 순차 재전송·순서 변경·명시적 종료 및 중복/변조 레코드 수신 경합 검사는 완료했다.
4. 고정 Agent 호스트와 신뢰할 수 있는 외부 관측 도구를 연결하여 우회 시험 실행.
5. 위 실제 실행 증거를 검토한 후 INS-11 및 배포 적합성 판정 갱신.
