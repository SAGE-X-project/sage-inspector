"""Execute frozen registry records and optional real stateful subject bindings."""
import argparse
import collections
import hashlib
import json
...
def main():
    ...
    try:
        ...
        manifest_raw=(ROOT/'vectors/0.10.0/registry-manifest.json').read_bytes()
        ...
        if len(entries)!=17: raise ValueError('manifest membership mismatch')
        scenarios=[]
        state_subject=None
        for file in sorted((ROOT/'vectors/0.10.0/registry-scenarios').glob('*.json')):
        ...
        if len(scenarios)!=17 or entries:
        ...
        summary=dict(schema_version=1,protocol_version='0.10.0',bundle='registry-inspection',subject=subject,state_subject=state_subject,status=status,
                     ...
                     scope='DID/PoP API and signature projections, with independent registry/Card contracts. Missing current registry/authority bindings are not certified. No deployed chain, origin or revocation latency measurement.')
