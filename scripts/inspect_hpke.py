"""Execute frozen HPKE schedules and optional real stateful subject bindings."""
import argparse
import collections
import hashlib
import json
...
NAMES=('hpke-primitives','hpke-schedule')
...
def main():
    ...
    try:
        ...
        scenarios=[]
        state_subject=None
        for file in sorted((ROOT/'vectors/0.10.0/hpke-scenarios').glob('*.json')):
            raw=file.read_bytes();fixture=json.loads(raw)
            item={'id':fixture['id'],'fixture_sha256':sha(raw),'status':'NOT_RUN','steps':len(fixture['steps']),'reason':'No stateful core binding supplied; schema1 adapters are not scenario adapters.'}
            if state_adapter:
                ...
                if report['fixture_sha256']!=sha(raw) or report['case_id']!=fixture['id'] or len(report['steps'])!=len(fixture['steps']):
                    raise ValueError('scenario identity/membership mismatch')
                if r.returncode!={'PASS':0,'FAIL':1,'INCOMPLETE':3}[report['status']]:
        ...
        summary=dict(schema_version=1,protocol_version='0.10.0',bundle='hpke-inspection',subject=subject,state_subject=state_subject,status=status,
                     primitive_counts=dict(counts),suites=reports,rules=rules,scenarios=scenarios,evidence_files=evidence,
                     scope='Fixed HPKE derivation and completion projections. State support and security assurance are separate; no formal proof, entropy/erasure or full protocol certification.')
