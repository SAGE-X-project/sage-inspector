...
|---|---|---|
| Go | `be621819d8e4f3a51895ccf469617a0aa70ee81c` | registry010.Source/Gate, guard010.Authority/IntentPolicy/Component, MCPWireSender/MCPEndpoint/Client |
| Rust | `ad30c6ade287765ac96a500699469599b9696fcd` | registry010::Source/Gate, guard010::Authority/IntentPolicy/Component, MCPWireSender/MCPEndpoint/Client |
...
| 전송 / 어댑터 | RPC는 구조·UUID·응답 상관관계를 검사한다. 비HTTP MCPSessionCall이 인증 신원·exact RPC 바이트·별도 outer/inner ID를 연결한다. 실제 Guard dispatch/client 및 운영 연결은 추가 검증 대상이다. | 인증된 initialize/version과 peer, 전체 바이트·invocation ID 결합, outer/inner 신원 일치, 별도 outer replay 및 연결 수명 |
...
[별도 실행 보고](guard-registry010.md)로 검증했다. MCP 보호 전송은
[세션 바인딩 보고](mcp-session010.md)로 검증했고 다음은 실제 Guard dispatch/client 결합이다.
