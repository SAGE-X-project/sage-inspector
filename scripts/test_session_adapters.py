...
for executable in sys.argv[1:]:
    for op in ('sage.session.record.open','sage.session.record.seal','sage.session.record.export'):
        c=next(c for c in suite['cases'] if c['operation']==('sage.session.record.seal' if op.endswith('export') else op))
        ...
        if op.endswith(('seal','export')): mutations += [('plaintext',None),('plaintext',{'byte':97,'length':8388574})]
