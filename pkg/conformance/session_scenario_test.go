...
func TestSessionStatefulFixturesLoad(t *testing.T) {
	paths, err := filepath.Glob("../../vectors/0.10.0/session-scenarios/*.json")
	if err != nil || len(paths) != 37 {
		t.Fatalf("missing Session lifecycle fixtures: %v", err)
	}
	for _, path := range paths {
		t.Run(filepath.Base(path), func(t *testing.T) {
			...
			if len(s.Steps) < 3 || s.Steps[0].Operation != "control.session.create" {
