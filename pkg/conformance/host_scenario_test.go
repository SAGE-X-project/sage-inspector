...
func TestHostStatefulFixturesLoad(t *testing.T) {
	paths, err := filepath.Glob("../../vectors/0.10.0/host-scenarios/*.json")
	if err != nil || len(paths) != 8 {
		t.Fatalf("missing host boundary fixtures: %v", err)
	}
	for _, path := range paths {
		t.Run(filepath.Base(path), func(t *testing.T) {
			...
			if len(s.Steps) < 3 || s.Steps[0].Operation != "control.host.setup" {
