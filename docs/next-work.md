...
> 2026-09-24 이후 단계 전환은 [SAGE 0.10.0 작업 순서 기준선](execution-order.md)을
...
현재 0.10.0 코어 수명주기 재검증: [INS-11 코어 증거](core-lifecycle-evidence.md). 구형 `SecureSession`의 과거 FAIL과 현행 `RecordSession010`의 종료·수신 PASS를 분리했다. 만료/admission, 늦은 완료, replay/quarantine 복구, reservation/close의 안전한 코어 테스트 및 로컬 프로세스 실행 결과를 고정했다. 역사적 lifecycle 37건 NOT_RUN과 전체 INS-11 NOT_ESTABLISHED는 유지한다. 고정 순서의 다음 작업은 실제 Registry Source와 체인 관측이다.
...
통합 MCP 제안의 [71개 사례 카탈로그](mcp-consolidated-catalog.md)를 별도로 고정했다.
원본 40개·추가 18개·보완 13개를 출처와 함께 추적하고 문서 해시 및 미실행 상태를
검사한다. 기존 40개 사례 모델과 과거 증거는 유지하며, 새 카탈로그 검사는 실제
프로토콜 실행을 뜻하지 않는다. 다음은 통합 규범의 검토·채택 후 owner API와
실제 협상 검증이다. 외부 독립 검토와 lifecycle 37개 NOT_RUN 상태는 유지한다.

통합 제안의 [owner 실행 허용·제한시간 모델](mcp-owner-model.md)을 추가했다.
...
Guard 서명 경계의 보정 suite는 P-256 low-S를 명시적으로 검사하고 secp256k1의
Keccak·low-S·공개키 복구까지 검산한다. 기존 결과 역할 P-256 두 벡터의 high-S
한계를 문서화하고 원본은 보존했다. 보정 14개 입력을 두 코어에서 28회 실행하며
MCP 전체 알고리즘·등록정보 지원 판정은 계속 별도 미완료 범위다.


Canonical Guard 증거 재검사기를 저장소와 CI에 추가했다. 고정 fixture·코어 revision·
신뢰된 빌드 바이너리 해시와 28개 요청/응답 원문을 대조하고 누락·중복·변경 및
